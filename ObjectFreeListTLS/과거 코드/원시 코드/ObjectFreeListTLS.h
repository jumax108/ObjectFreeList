...
CObjectFreeListTLS<T>::CObjectFreeListTLS(HANDLE heap, bool runConstructor, bool runDestructor){
	...
	_dataToNodePtr = (unsigned __int64)&tempNode - (unsigned __int64)&tempNode._data;
...
typename T* CObjectFreeListTLS<T>::allocObject(){
	...
	stAllocTlsNode<T> *node = &chunk->_nodes[allocNodeIdx];
	T* allocData = &node->_data;
